<?php
    ...
    {
        $DeleteCustomer = "DELETE FROM `master_inventory` WHERE `InventoryID` = '".$_REQUEST['id']."'";
        mysqli_query($DeleteCustomer);
        $DeleteCustomer = "DELETE FROM `master_inventory_child` WHERE `InventoryID` = '".$_REQUEST['id']."'";
        mysqli_query($DeleteCustomer);
...
<section class="ng-scope">
    <div ui-view="" autoscroll="false" ng-class="app.views.animation" class="app ng-scope ng-fadeInLeft2">
        <div ui-view="" ng-class="app.views.animation" class="ng-scope ng-fadeInLeft2">
            <div class="app-view-header ng-scope">View Supplier Price</div>
            ...
            <div ng-controller="TablexEditableController as xtable" class="container-fluid ng-scope">
                <table class="table table-bordered" id="example" >
                    <thead>                    
                        <tr style="font-weight: bold">
                            ...
                            <th>Date</th>
                            <th>Supplier Name</th>
                            <th class="text-right">Cycle Total Price</th>
                            <th class="text-right">Spare Total Price</th>
                            <th class="text-right">Tran. Price</th>
                    ...
                    <tbody>
                        <?php
                            $selectCustomer = "SELECT * FROM `master_inventory`,master_supplier WHERE master_inventory.SupplierID = master_supplier.SupplierID Order by InventoryID Desc";
                            ...
                            if($NumselectCustomer>0)
                            {
                                ...
                                {        
                                    ...
                                    $CyclePrice = 0;
                                    $SparePrice = 0;
                                    $selectInventory = "SELECT SUM((OrgBasicPrice * OrgQuantity) + OrgTaxPrice - OrgDiscountPrice - OrgSchemePrice) as TotalPrice,Type,OrgQuantity from master_inventory_child WHERE InventoryID='".$RsselectCustomer['InventoryID']."' GROUP BY Type ORDER BY Type";
                                    $ObjselectInventory = mysqli_query($selectInventory);
                                    while($RsselectInventory = mysqli_fetch_array($ObjselectInventory))
                                    {
                                        if($RsselectInventory['Type']=="cycle")
                                        {
                                            if($RsselectInventory['TotalPrice']>0)
                                            {
                                                $CyclePrice = $RsselectInventory['TotalPrice'];    
                                            }
                                        }
                                        if($RsselectInventory['Type']=="spare")
                                        {
                                            if($RsselectInventory['TotalPrice']>0)
                                            {
                                                $SparePrice = $RsselectInventory['TotalPrice'];    
                                            }
                                        }
                                    }
                                    ?>
                                        <tr ng-repeat="user in xtable.users" class="ng-scope">
                                            <td><span><?php echo $RsselectCustomer['InventoryID'];?></span></td>
                                            <td><span><?php echo $RsselectCustomer['InvoiceNumber'];?></span></td>
                                            <td>
                                                <span>
                                                    <?php 
                                            ...
                                            <td class="text-right"><span><?php if($CyclePrice!=""){echo number_format(($CyclePrice),2,".",",");} else { echo "-";};?></span></td>
                                            <td class="text-right"><span><?php if($SparePrice!=""){echo number_format(($SparePrice),2,".",",");} else { echo "-";};?></span></td>
                                            <td class="text-right"><span><?php echo number_format(($RsselectCustomer['TransportationPrice']),2,".",",");?></span></td>
                                            <td class="text-right"><span><?php echo number_format((floatval($CyclePrice) + floatval($SparePrice) + floatval($RsselectCustomer['TransportationPrice'])),2,".",",");?></span></td>
                                            <td style="white-space: nowrap">
                                                <div ng-show="!rowform.$visible" class="buttons">
                                                    <a class="btn btn-sm btn-info" href="<?php echo PreURL;?>/inventory.php?action=edit&id=<?php echo $RsselectCustomer['InventoryID'];?>">
                                                        <em class="fa fa-pencil"></em>
                                                    </a>
                                                    <a class="btn btn-sm btn-danger" href="<?php echo PreURL;?>/viewinventory.php?action=delete&id=<?php echo $RsselectCustomer['InventoryID'];?>">
                ...
                <div>&nbsp;</div>
                <a href="<?php echo PreURL;?>/inventory.php?action=add" class="btn btn-default">Add Inventory</a>
            </div>
        </div>
   </div>
</section> 
