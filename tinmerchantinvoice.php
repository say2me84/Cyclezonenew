<?php
    ...
    {
        $title = "Add TIN Merchant Invoice";    
    }
...
//        exit;
        ...
        {   
            ...
            $selectInvoiceNo = "SELECT MAX(InvoiceNo) as InvoiceNo from master_invoice WHERE Type='tinmerchant'";
            ...
            header("location:tinmerchantviewinvoice.php?id=".$InvoiceID);
        }
        else
        {
            $TinNumberBox = 1;                
    ...
    {
        $title = "Edit Tin Merchant Invoice";    
        ...
        <div ui-view="" ng-class="app.views.animation" class="ng-scope ng-fadeInLeft2">
            ...
            <div>
                ...
                <?php
                    if($_REQUEST['action']=="edit" && $_REQUEST['id']!="")
                    {
                        echo "MRTIN".sprintf("%04d", $RsselectInvoice['InvoiceNo']);
                    }
                    else
                    {
                        $selectInvoiveIDNext = "SELECT MAX(InvoiceNo) as InvoiceNo FROM `master_invoice` WHERE Type='tinmerchant'";
                        $ObjselectInvoiveIDNext = mysqli_query($selectInvoiveIDNext);
                        $RsselectInvoiveIDNext = mysqli_fetch_array($ObjselectInvoiveIDNext);
                        echo "MRTIN".sprintf("%04d",intval($RsselectInvoiveIDNext['InvoiceNo'])+intval(1));   
                    }
                            ...
                            <div id="TinNumberBox">
                                <?php
                                    if($_REQUEST['action']=="edit" && $_REQUEST['id']!="")
                                    {
                                        ...
                                        $selectMerchant = "SELECT * FROM `master_merchant` WHERE MerchantID = '".$RsselectInvoice['CustomerID']."'";        
                                        $ObjselectMerchant = mysqli_query($selectMerchant);
                                        $RsselectMerchant = mysqli_fetch_array($ObjselectMerchant);
                                        if($RsselectMerchant['TinNumber']!="")
                                        {
                                            if($RsselectInvoice['IsTinNumber']=="1")
                                            {
                                            ...
                                            ?>
                                                <div style="padding:10px 0px;text-align: center;">
                                                    <input <?php echo $checked;?> type="checkbox" name="TinNumberBox" id="TinNumberBox" value="1"> is Tin Number Invoice?
                                                </div>                
                                            <?php
                                        }
                                    }
...
//                                                                                                $RsSelectPrice = mysqli_fetch_array($objSelectPrice);        
                                                            ...
                                                            ?>
                                                            <td class="text-left">
                                                                <input type="text" value="<?php echo number_format($RsselectInvoiceChild['Price'],2,".","");?>" class="text-right form-control shadow-z1 Price" name="Price[<?php echo $RsselectInvoiceChild['InvoiceChildID'];?>]">
                                                            </td>
                                                            <?php
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#MerchantID').change(function() {
            ...
            {
                $.ajax({
                    url: 'ajax.php?filter=GetMerchantInvoiceForm&MerchantID='+$(this).val()+'&IsTin=1',
                    success: function(html) 
                    {
                        $('#divMerchantInfo').html(html);                    
                    }
        ...
        $('#FrmPurchaseOrder').submit(function() {
            ...
            if(MerchantID=="new")
            {
                ...
                if($('#TinNumber').val()=="")
                {
                    $("#TinNumber").attr('style', 'border:1px solid red !important');
                    Error = true;
                }
                else
                {
                    $('#TinNumber').attr('style', 'border:1px solid #ccc !important');
                }
