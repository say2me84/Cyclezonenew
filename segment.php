<?php
    ...
    if($_POST)
    {
        ...
        if($_REQUEST['action']=="add")
        {
            $selectProduct = "SELECT * FROM `master_segment` WHERE SegmentName='".$_POST['SegmentName']."'";
            $ObjselectCustomer = mysqli_query($selectProduct);
            $NumselectCustomer = mysqli_num_rows($ObjselectCustomer);
            if($NumselectCustomer==0)
            {
                $InsertTax = "INSERT INTO `master_segment` (`SegmentName`)VALUES ('".$_POST['SegmentName']."');";
                mysqli_query($InsertTax);
                $Msg = "Segment Added Successfully";    
            }
            else
            {
                $Msg = "Segment is already exist";                
            }
        }
        else
        {
            $selectProduct = "SELECT * FROM `master_segment` WHERE SegmentName='".$_POST['SegmentName']."' AND SegmentID<>'".$_REQUEST['id']."'";
            $ObjselectCustomer = mysqli_query($selectProduct);
            $NumselectCustomer = mysqli_num_rows($ObjselectCustomer);
            if($NumselectCustomer==0)
            {
                $UpdateProduct = "UPDATE `master_segment` SET `SegmentName` = '".$_POST['SegmentName']."' WHERE `SegmentID` ='".$_REQUEST['id']."';";
    ...
    {
        $title = "Edit Segment";    
        $selectProduct = "SELECT * FROM `master_segment` WHERE SegmentID='".$_REQUEST['id']."'";
        $ObjselectCustomer = mysqli_query($selectProduct);
                        ...
                        <form action="" method="post">
                            <fieldset>
                                <div class="form-group">
                                    <label for="input-id-1" class="col-sm-2 control-label">Segment Name</label>
                                    <div class="col-sm-10">
                                        <input value="<?php echo $RsselectCustomer['SegmentName'];?>" id="SegmentName" name="SegmentName" placeholder="Segment Name" type="text" class="form-control shadow-z1">
                                    </div>
                                </div>
                            </fieldset>
                            <fieldset>
