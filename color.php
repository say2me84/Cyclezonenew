<?php
    ...
    if($_POST)
    {
        ...
        if($_REQUEST['action']=="add")
        {
            $selectProduct = "SELECT * FROM `master_color` WHERE ColorName='".$_POST['ColorName']."'";
            $ObjselectCustomer = mysqli_query($selectProduct);
            $NumselectCustomer = mysqli_num_rows($ObjselectCustomer);
            if($NumselectCustomer==0)
            {
                $InsertTax = "INSERT INTO `master_color` (`ColorName`)VALUES ('".$_POST['ColorName']."');";
                mysqli_query($InsertTax);
                $Msg = "Color Added Successfully";    
            }
            else
            {
                $Msg = "Color is already exist";                
            }
            
        }
        else
        {
            $selectProduct = "SELECT * FROM `master_color` WHERE ColorName='".$_POST['ColorName']."' AND  ColorID<>'".$_REQUEST['id']."'";
            $ObjselectCustomer = mysqli_query($selectProduct);
            $NumselectCustomer = mysqli_num_rows($ObjselectCustomer);
            if($NumselectCustomer==0)
            {
                $UpdateProduct = "UPDATE `master_color` SET `ColorName` = '".$_POST['ColorName']."' WHERE `ColorID` ='".$_REQUEST['id']."';";
    ...
    {
        $title = "Edit Color";    
        $selectProduct = "SELECT * FROM `master_color` WHERE ColorID='".$_REQUEST['id']."'";
        $ObjselectCustomer = mysqli_query($selectProduct);
                        ...
                        <form action="" method="post">
                            <fieldset>
                                <div class="form-group">
                                    <label for="input-id-1" class="col-sm-2 control-label">Color Name</label>
                                    <div class="col-sm-10">
                                        <input value="<?php echo $RsselectCustomer['ColorName'];?>" id="ColorName" name="ColorName" placeholder="Color Name" type="text" class="form-control shadow-z1">
                                    </div>
                                </div>
                            </fieldset>
                            <fieldset>
